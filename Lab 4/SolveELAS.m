function [d strainGLO stressGLO  React posgp DOFl Kll Mll] = SolveELAS(K,M,Fb,Ftrac,dR,DOFr,COOR,CN,TypeElement,celasglo,typePROBLEM,celasgloINV)
...
% Right-hand side
F = Fb + Ftrac ;
% Set of nodes at which displacements are unknown
DOFl = 1:nnode*ndim ;
DOFl(DOFr) = [];
Fl = F(DOFl);
Fr = F(DOFr);
...
React(DOFr) = Krl*dL + Krr*dR - Fr;

%%%% COmputation of strain and stress vector at each gauss point
disp('Computation of stress and strains at each Gauss point')
[strainGLO stressGLO posgp]= StressStrains(COOR,CN,TypeElement,celasglo,d,typePROBLEM,celasgloINV) ;
