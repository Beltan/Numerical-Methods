...
for e = 1 : nelem
    CNloc  =  CN( e , : );
    fe = fNOD ( CNloc ) ; %Source function evaluated at the nodes of each element
    Xe = COOR( CNloc , : )'; %Coordinates of the node for a given element
    Fse = ComputeFseVector(fe , weig , shapef , dershapef , Xe); %Computation of the elemental source flux vector
    for a = 1:nnodeE;
        A = CN(e, a);
        Fs(A) = Fs(A) + Fse(a);
    end    
